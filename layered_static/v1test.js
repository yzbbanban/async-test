...
async function render(layout, _blockNum, masterArtTokenId) {
	...
	for (var i = 0; i < 6; i++) {
		...
		if (KEY_STATES in layer) {
			var uriIndex = await readIntProperty(layer, KEY_STATES, "Layer Index", masterArtTokenId);
		...
		if (KEY_VISIBLE in layer) {
			var isVisible = (await readIntProperty(layer, KEY_VISIBLE, "Layer Visible", masterArtTokenId)) === 1;
			if (isVisible === false) {
		...
		if (currentImage == null) {
			...
			currentImage = await renderLayer(currentImage, layout, layer, layerImage, masterArtTokenId);
		}
...
async function readIntProperty(object, key, label, masterArtTokenId) {
	console.log('==========readIntProperty==========');
	...
	if (typeof value === "object") {
		...
		if (tokenId in controlTokenCache) {
			...
			if (blockNum >= 0) {
				controlLeverResults=[1,2,2,2,2,2,2,2,2,2,2]
			} else {
				controlLeverResults=[1,2,2,2,2,2,2,2,2,2,2]
			}
		...
		console.log('currentLeverValue'+ currentLeverValue);
...
async function renderLayer(currentImage, layout, layer, layerImage, masterArtTokenId) {
	...
	if (KEY_SCALE in layer) {
		var scale_x = (await readIntProperty(layer[KEY_SCALE], KEY_X, "Layer Scale X", masterArtTokenId)) / 100;
		var scale_y = (await readIntProperty(layer[KEY_SCALE], KEY_Y, "Layer Scale Y", masterArtTokenId)) / 100;
	...
	if (KEY_FIXED_ROTATION in layer) {
		var rotation = await readIntProperty(layer, KEY_FIXED_ROTATION, "Layer Fixed Rotation", masterArtTokenId);
	...
	if (KEY_MIRROR in layer) {
		var shouldMirrorHorizontal = ((await readIntProperty(layer[KEY_MIRROR], KEY_X, "Mirror X", masterArtTokenId)) == 1);
		var shouldMirrorVertical = ((await readIntProperty(layer[KEY_MIRROR], KEY_Y, "Mirror Y", masterArtTokenId)) == 1);
	...
	if (KEY_ANCHOR in layer) {				
		...
		if (typeof anchorLayerId === "object") {
			// TODO test this
			var anchorLayerIndex = await readIntProperty(layer, KEY_ANCHOR, "Anchor Layer Index", masterArtTokenId);
	...
	if (KEY_FIXED_POSITION in layer) {
		...
		x = await readIntProperty(layer[KEY_FIXED_POSITION], KEY_X, "Layer Fixed Position X", masterArtTokenId);
		y = await readIntProperty(layer[KEY_FIXED_POSITION], KEY_Y, "Layer Fixed Position Y", masterArtTokenId);
	} else {
		// relative position adjusts xy based on the anchor
		if (KEY_RELATIVE_POSITION in layer) {
			relativeX = await readIntProperty(layer[KEY_RELATIVE_POSITION], KEY_X, "Layer Relative Position X", masterArtTokenId);
			relativeY = await readIntProperty(layer[KEY_RELATIVE_POSITION], KEY_Y, "Layer Relative Position Y", masterArtTokenId);
		}

		// relative rotation orbits this layer around an anchor
		if (KEY_ORBIT_ROTATION in layer) {
			var relativeRotation = await readIntProperty(layer, KEY_ORBIT_ROTATION, "Layer Orbit Rotation", masterArtTokenId);
	...
	if (KEY_COLOR in layer) {
		if (KEY_RED in layer[KEY_COLOR]) {
			var red = await readIntProperty(layer[KEY_COLOR], KEY_RED, "Layer Color Red", masterArtTokenId); 
		...
		if (KEY_GREEN in layer[KEY_COLOR]) {
			var green = await readIntProperty(layer[KEY_COLOR], KEY_GREEN, "Layer Color Green", masterArtTokenId); 
		...
		if (KEY_BLUE in layer[KEY_COLOR]) {
			var blue = await readIntProperty(layer[KEY_COLOR], KEY_BLUE, "Layer Color Blue", masterArtTokenId); 
		...
		if (KEY_HUE in layer[KEY_COLOR]) {
			var hue = await readIntProperty(layer[KEY_COLOR], KEY_HUE, "Layer Color Hue", masterArtTokenId); 
		...
		if (KEY_ALPHA in layer[KEY_COLOR]) {
			var alpha = await readIntProperty(layer[KEY_COLOR], KEY_ALPHA, "Layer Color Alpha", masterArtTokenId); 
